...
class AssetDetail(models.Model):
    ...
    image = models.FileField(upload_to='AssetDetails', null=True, blank=True)
...
class AssetMaintance(models.Model):
    ...
    @property
    def maintanance_date_has_passed(self):
        if self.mainannce_date < datetime.now().date():
            return True
        return False
    class Meta:
...
class HouseDetail(models.Model):
    ...
    image = models.FileField(upload_to='HouseDetails', null=True, blank=True)
    asset = models.ForeignKey(AssetDetail, on_delete= models.CASCADE)
    name = models.CharField(max_length=100)
    date_created = models.DateTimeField(auto_now_add=True)
    date_updated = models.DateTimeField(auto_now=True)
...
class AssetUserDescription(models.Model):
    ...
    aplication_type = models.CharField(max_length = 100, null = True, blank = True)
